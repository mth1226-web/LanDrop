...
1. 各PCでLanDropを起動する（同じWi-Fi/LANに接続していること）。設定画面で「共有フォルダ」を確認・変更できる（デフォルトはドキュメント内の「LanDrop共有」フォルダ）
2. しばらくすると左のPC一覧に他の端末が表示される。一番上には自分自身も「（自分）」として表示される
3. PCを選ぶと、そのPCの共有フォルダの中身が右側に表示される。フォルダはクリックで移動でき、上部のパンくずで階層を戻れる
4. 「アップロード」ボタンまたはファイルのドラッグ&ドロップで、選んでいるPCの共有フォルダにファイルを送り込める（自分のPCを選んでいる場合はローカルにコピーされる）
5. ファイルの「ダウンロード」ボタンで、選んでいるPCの共有フォルダから自分の「ダウンロード保存先」（設定で変更可、デフォルトはダウンロードフォルダ）に取得できる
6. 「新しいフォルダ」でフォルダ作成、「名前変更」でファイル/フォルダのリネームができる
